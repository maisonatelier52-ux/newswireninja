      ...
      <section className="lg:col-span-2">
        {latestArticles[0] && (
          ...
          <h1 className="sr-only">
            {`Newswire Ninja — Independent U.S. News, Business & Investigations - ${
              latestArticles[0].title.length + 37 > 70 
                ? latestArticles[0].title.slice(0, 70 - 37) + "..." 
                : latestArticles[0].title
            }`}
          </h1>
            <div className="relative h-[240px] sm:h-[320px] lg:h-[420px] w-full">
            ...
            <div className="mt-5">
              <Link href={`${latestArticles[0].category}/${latestArticles[0].slug}`} title={`Read the latest article: ${latestArticles[0].title}`}>
                {/* Ensure <h2> instead of skipping directly to <h3> */}
                <h2 className="font-serif text-xl sm:text-3xl lg:text-4xl font-bold leading-tight hover:text-blue-600 hover:underline">
                  {latestArticles[0].title}
                </h2>
              </Link>
              <p className="sr-only">
        ...
        {/* Change this to <h3> for the subsequent sections */}
        <h3 className="mt-4 text-sm text-gray-600 max-w-3xl">
          {generateHeroContext(latestArticles[0]?.title)}
        </h3>
        <p className="mt-2 text-sm text-gray-700 max-w-4xl">
          Newswireninja is an independent digital news organization delivering trusted U.S. news, business reporting, and in-depth investigations worldwide.
        </p>
      </section>
          ...
          <article className="space-y-3">
            ...
            <div className="hidden sm:block">
              <Link href={`${latestArticles[1].category}/${latestArticles[1].slug}`} title={latestArticles[1].title}>
                <h3 className="font-serif text-lg sm:text-2xl font-semibold leading-snug hover:text-blue-600 hover:underline">
                  {latestArticles[1].title.length > 30
                    ? `${latestArticles[1].title.slice(0, 30)}...`
                    : latestArticles[1].title}
                </h3>
              </Link>
              <p className="text-sm sm:text-base text-gray-700">{latestArticles[1].excerpt}</p>
              <div className="flex items-center justify-between text-xs sm:text-sm text-gray-500">
                <div className="flex items-center gap-1">
                  <GoClock />
                  <span>{latestArticles[1].readTime}</span>
